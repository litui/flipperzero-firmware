...
static DTMFDolphinApp* app_alloc() {
    ...
    app->max_samples = 2 * 1024;
    app->sample_count = 256;
    app->sample_buffer = malloc(sizeof(uint8_t) * app->sample_count);
    app->queue = furi_message_queue_alloc(10, sizeof(DTMFDolphinEvent));
    app->volume = 1.0f;
